...
function getProxiedUrl(url) {
  if (!url) return null
  if (isCanvaUrl(url)) return `/api/canva-proxy?url=${encodeURIComponent(url)}`
  return url
}

function CanvaCell({ post, onClick }) {
  const [state, setState] = useState('loading') // loading | image | fallback | error
  const [imgSrc, setImgSrc] = useState(null)
  const [canvaUrl, setCanvaUrl] = useState(null)

  useEffect(() => {
    const proxiedUrl = `/api/canva-proxy?url=${encodeURIComponent(post.imageUrl)}`
    fetch(proxiedUrl)
      .then(async res => {
  ...
  if (state === 'loading') {
    return (
      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f3ff' }}>
        <div style={{ width: 20, height: 20, border: '2px solid #e0e7ff', borderTop: '2px solid #7c3aed', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
      </div>
    )
  }

  if (state === 'image' && imgSrc) {
    return (
      <img
        src={imgSrc}
        alt={post.title}
  ...
  // Fallback : placeholder cliquable avec lien Canva
  return (
    <div
      style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f5f3ff', cursor: 'pointer', gap: 4, padding: 8 }}
      onClick={() => canvaUrl ? window.open(canvaUrl, '_blank') : onClick()}
    >
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z" fill="#7c3aed" opacity="0.6"/>
      </svg>
      <span style={{ fontSize: 9, color: '#7c3aed', textAlign: 'center', fontWeight: 600 }}>Canva</span>
      <span style={{ fontSize: 8, color: '#a78bfa', textAlign: 'center' }}>Cliquer pour ouvrir</span>
    </div>
  )
}
              ...
              {post ? (
                <>
                  {post.imageUrl && isCanvaUrl(post.imageUrl) ? (
                    <CanvaCell post={post} onClick={() => setPreview(post)} />
                  ) : post.imageUrl ? (
                    <img
                      src={post.imageUrl}
                      alt={post.title}
                    ...
                    <div style={{ ...styles.placeholder }} onClick={() => setPreview(post)}>
                  ...
                  <div style={styles.hoverOverlay} onClick={() => setPreview(post)}>
                    <span style={styles.hoverTitle}>{post.title}</span>
                    {post.date && (
                      <span style={styles.hoverDate}>
        ...
        <div style={styles.modalBg} onClick={() => setPreview(null)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            {preview.imageUrl && isCanvaUrl(preview.imageUrl) ? (
              <div style={{ width: '100%', aspectRatio: '1', background: '#f5f3ff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
                <img src={`/api/canva-proxy?url=${encodeURIComponent(preview.imageUrl)}`} alt={preview.title}
            ...
            <div style={styles.modalInfo}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
                <span style={{
                  ...styles.statusBadge, position: 'static', fontSize: 12, padding: '3px 8px',
                  background: STATUS_CONFIG[preview.status].bg, color: STATUS_CONFIG[preview.status].color
                }}>
...
const styles = {
  ...
  cell: { aspectRatio: '1', position: 'relative', background: '#f3f4f6', overflow: 'hidden', cursor: 'grab', borderRadius: 2 },
  cellDragging: { opacity: 0.4, cursor: 'grabbing' },
  img: { width: '100%', height: '100%', objectFit: 'cover', display: 'block', cursor: 'pointer' },
  placeholder: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, background: '#f9fafb', cursor: 'pointer' },
  ...
  hoverOverlay: { position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: 8, opacity: 0, transition: 'opacity 0.18s', zIndex: 5, cursor: 'pointer' },
  hoverTitle: { color: '#fff', fontSize: 11, fontWeight: 500, textAlign: 'center', lineHeight: 1.3 },
  hoverDate: { color: 'rgba(255,255,255,0.7)', fontSize: 10, marginTop: 2 },
  emptyCell: { width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#d1d5db', fontSize: 20, cursor: 'default' },
