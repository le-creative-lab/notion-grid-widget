...
export async function getDatabasePosts(token, databaseId) {
  ...
  return response.results.map((page) => {
    ...
    // ✅ Pinné (max 3 gérés côté widget)
    const pinnedProp = props['Pinné'] || props['Pinned'] || props['Pin'] || props['pin']
    const pinned = pinnedProp?.checkbox === true

    // ✅ Masqué
    const hiddenProp = props['Masqué'] || props['Hidden'] || props['Masque'] || props['masqué']
